...
function Keyboard:Init()
    ...
    UserInputService.InputBegan:connect(function(key)
        if key.KeyCode == Enum.KeyCode.ButtonR2 or key.UserInputType == Enum.UserInputType.MouseButton2 then
            if self.SelectedKey then
                firetouchinterest(LocalPlayer.Character.PrimaryPart, self.SelectedKey, 0)
                task.wait()
                firetouchinterest(LocalPlayer.Character.PrimaryPart, self.SelectedKey, 1)
            end
        end
    end)
end
